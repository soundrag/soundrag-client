...
이러한 문제점을 해결하기 위해 프로젝트 화면의 좌표를 **변환**하여 사용자의 모니터 해상도와 비율에 맞는 독립적인 좌표를 설정합니다.
...
2D 화면에서는 깊이 즉, z축 정보를 직접 제공하지 않기 때문에 3D 모델의 좌표를 구하기 위해서는 깊이를 알아야 합니다.

화면으로부터 광선을 발사하여 3D 공간에서의 z축 깊이을 계산합니다. 이를 통해 마우스 위치에 대응하는 3D 공간의 변환된 [x, y, z] 좌표를 얻을 수 있습니다.

<div align="center">
 <img width="600" alt="스크린샷 2024-10-24 오전 8 23 24" src="https://github.com/user-attachments/assets/3aeb966e-a9b8-40d6-8cc6-0da4751169df">
...
#### (3) 3D 모델과 마우스의 움직임 연결하기

> 3D 모델과 마우스의 움직임을 연결하기 위해 마우스 이벤트 연결해야 합니다.

- 3D 모델과 마우스 이벤트를 연결해야 하는 이유
  마우스의 움직임으로 3D 모델의 위치를 변경하기 위해서는 마우스의 드래그 앤 드롭 이벤트가 필요합니다.

- 3D 모델과 마우스의 움직임을 연결하는 방법
  실시간으로 추적하는 변환된 [x, y, z] 좌표를 마우스 이벤트와 연결합니다.
