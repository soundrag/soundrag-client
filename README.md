...
스피커의 **왼쪽, 오른쪽 배치**를 구분하기 위해 삼각함수 중 하나인 **cos** 함수를 사용하였습니다.
...
먼저, 리스너를 기준으로 스피커와 `[x, z]` 좌표 거리로 **각도**를 구합니다.
...
다음, 구한 **각도**를 **cos** 함수에 대입하여 계산된 **값**으로 **위치**(좌, 우)를 설정합니다.
...
마지막으로 값이 `-1`에 가까우면 **왼쪽** 볼륨을 강조하고 `1`에 가까우면 **오른쪽** 볼륨을 강조합니다.
