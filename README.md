...
- [3️⃣ 챌린지](#3%EF%B8%8F%E2%83%A3-%EC%B1%8C%EB%A6%B0%EC%A7%80-1)
  ...
  * [② 자동 저장](#%E2%91%A1-%EC%9E%90%EB%8F%99-%EC%A0%80%EC%9E%A5)
  ...
  * [③ 메시지 알림](#%E2%91%A2-%EB%A9%94%EC%8B%9C%EC%A7%80-%EC%95%8C%EB%A6%BC)
...
- [① Soundrag는 무엇인가요?](#①-Soundrag는-무엇인가요)
- [② Soundrag는 왜 만들었나요?](#②-soundrag는-왜-만들었나요)
...
- [① 공간 음향](#①-공간-음향)
  - [위치에 따라 달라지는 소리의 원리](#위치에-따라-달라지는-소리의-원리)
  - [API를 활용하여 공간 음향 구현하기](#api를-활용하여-공간-음향-구현하기)
  - [사용자와 스피커의 위치에 따른 공간 음향 구현하기](#사용자와-스피커의-위치에-따른-공간-음향-구현하기)
  - [스피커의 수평적 움직임에 따른 공간 음향 구현하기](#스피커의-수평적-움직임에-따른-공간-음향-구현하기)
  - [스피커의 수직적 움직임에 따른 공간 음향 구현하기](#스피커의-수직적-움직임에-따른-공간-음향-구현하기)
- [② 자동 저장](#②-자동-저장)
  - [서버 요청 없이 클라이언트에서 자동 저장하기](#서버-요청-없이-클라이언트에서-자동-저장하기)
  - [새로고침해도 지워지지 않는 상태 관리](#새로고침해도-지워지지-않는-상태-관리)
- [③ 메시지 알림](#③-메시지-알림)
  - [사용자에게 메시지를 알려주는 기준](#사용자에게-메시지를-알려주는-기준)
  - [사용자에게 반드시 알려줘야 하는 메시지](#사용자에게-반드시-알려줘야-하는-메시지)
  - [사용자에게 가볍게 알려줘도 되는 메시지](#사용자에게-가볍게-알려줘도-되는-메시지)
...
- Google 계정을 연동하여 사용 가능합니다.
- 사용자는 최적의 스피커 위치를 저장할 수 있습니다.
...
1. 소리는 인간의 머리와 귀의 구조로 인해 귀에 도달할 때 미세한 시간 차이와 주파수 응답 변화를 겪습니다. 이러한 현상을 수학적으로 모델링한 **HRTF**는 소리가 특정 위치에서 특정 방향으로 귀에 도달할 때의 주파수 응답과 시간 차이를 나타냅니다. 이를 통해 스피커와 사용자의 위치 변화에 따라 소리의 방향성과 거리감을 구현하였습니다.
   ...
   그림과 같이 **HRTF**는 사용자의 [x, y, z] 값과 스피커의 [x, y, z] 값을 비교하여 거리가 멀어질 때 마다 소리가 줄어들고, 거리가 가까워질 때 소리가 커지는 **거리 감쇠**라는 개념을 사용합니다.
...
2. 스피커의 수평적 움직임에 따라 소리는 **IID** 현상이 발생합니다. **IID**는 소리가 두 귀에 도달할 때의 음파의 에너지 수준 차이를 의미합니다. 예를 들어, 스피커가 왼쪽에 위치하면 왼쪽 귀에는 직접적인 소리가 더 강하게 도달하고, 오른쪽 귀에는 머리가 막고 있어 소리가 약해집니다. 이로 인해 왼쪽 소리가 더 크게 들리며, 뇌는 이 강도 차이를 이용해 소리의 방향을 인지합니다. 이러한 현상을 고려하여 스피커의 수평적 방향성을 구현하였습니다.
   ...
   **IID**는 사용자의 [x, z] 값을 기준으로 스피커와의 [x, z] 값을 비교하여 거리를 활용한 각도를 계산한 값으로 왼쪽 오른쪽을 구분합니다.
...
`BiquadFilterNode`는 소리의 특정 주파수 음역대를 조절하여 음색을 변경할 수 있는 노드입니다. 스피커의 수직적 움직임을 통해 <ins>바닥에 가까워질수록 저주파 음역대를 강조하고, 천장에 가까워질수록 고주파 음역대를 강조</ins>하였습니다.
...
1. 위치 정보가 보안에 민감하지 않은 정보라고 판단

2. 실수로 브라우저나 탭을 닫은 경우, 데이터를 유지할 필요가 있다고 판단

위와 같은 이유로 localStorage를 사용하기로 결정하였습니다. 이러한 방식으로 자동 저장 기능을 구현하여 사용자가 작업 도중에 실수로 페이지를 새로고침하거나 브라우저를 닫아도 이전 작업 상태를 복원할 수 있게 구현하였습니다.
