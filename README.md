...
- [1. 기술 스택](#1-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D)
...
- [3. 추가 기능 구현](#3-%EC%B6%94%EA%B0%80-%EA%B8%B0%EB%8A%A5-%EA%B5%AC%ED%98%84)
  ...
  - [3-3. 3D 리소스 프리로드](#3-3-3d-%EB%A6%AC%EC%86%8C%EC%8A%A4-%ED%94%84%EB%A6%AC%EB%A1%9C%EB%93%9C)
    - [(1) 프리로드가 필요한 이유](#1-%ED%94%84%EB%A6%AC%EB%A1%9C%EB%93%9C%EA%B0%80-%ED%95%84%EC%9A%94%ED%95%9C-%EC%9D%B4%EC%9C%A0)
    - [(2) 프리로드는 리소스를 언제 불러오는 걸까?](#2-%ED%94%84%EB%A6%AC%EB%A1%9C%EB%93%9C%EB%8A%94-%EB%A6%AC%EC%86%8C%EC%8A%A4%EB%A5%BC-%EC%96%B8%EC%A0%9C-%EB%B6%88%EB%9F%AC%EC%98%A4%EB%8A%94-%EA%B1%B8%EA%B9%8C)
...
> 애플리케이션을 작동하는데 필요한 리소스가 느리게 로딩된다면 사용자 경험을 저해할 수 있습니다.

3D 프로젝트에서 필요한 리소스를 사용 시점에 로드하게 되면, 아래와 같은 문제점이 발생할 수 있습니다.
...
- 네트워크가 느린 경우, 3D 모델이나 텍스처가 로드되지 않아 **빈 화면**이 사용자에게 노출될 수 있습니다.

- 텍스처나 모델이 로드되지 않은 상태로 렌더링되어 **정상적으로 작동하지 않는 듯한 느낌**이 사용자에게 전달될 수 있습니다.

이를 해결하기 위해 **프리로드**를 통해 애플리케이션이 시작될 때 필요한 3D 리소스(모델, 텍스처)를 미리 불러오도록 구현하였습니다.

#### (2) 프리로드는 리소스를 언제 불러오는 걸까?

> **애플리케이션이 시작 단계**에서 3D 리소스를 미리 불러오고 로딩 시간을 확보하기 위해 진입 페이지를 구현하였습니다.

사용자가 애플리케이션을 시작할 때, 필요한 3D 리소스를 **프리로드**하도록 설정하였습니다. 이를 통해 네트워크 지연이나 로드 중 발생할 수 있는 빈 화면 노출을 방지하였습니다. 특히, 3D 모델과 텍스처는 사용자가 3D 화면에 진입하기 전 단계에서 미리 불러오도록 설계되어, 로드가 완료된 상태에서 콘텐츠가 즉시 렌더링되도록 처리하였습니다.

또한, **진입 페이지**를 사용하여 초기 로딩 시간을 확보함으로써 사용자는 로딩 과정을 시각적으로 인지할 수 있고 콘텐츠가 준비된 상태에서 원활하게 애플리케이션을 탐색할 수 있도록 하였습니다.
...
이와 같은 구현을 통해 애플리케이션의 안정적인 초기 로딩 환경을 제공하고 3D 화면으로의 전환 과정에서 발생할 수 있는 불편함을 최소화하여 **사용자 경험을 최적화**하였습니다.
